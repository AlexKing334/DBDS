...
def accedi():
    try:
        ...
        url = "https://localhost:30002/login"
...
def logout():
    try:
        url = "https://localhost:30002/logout"
...
def register():
    try:
        ...
        if has_no_spaces(username) or has_no_spaces(email) or has_no_spaces(password):
            ...
            return
        url = "https://localhost:30002/register"
...
def get_subscriptions():
    try:
        url = "https://localhost:30003/get_subscriptions"
        payload = {'token': user.getToken()}
        requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
...
def my_subscriptions():
    try:
        url = "https://localhost:30003/my_subscriptions"
        payload = {'token': user.getToken()}
        requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
        response = requests.get(url, json=payload, cert=(cert_, key_file), verify=False)
        result = response.json()
...
def delete_items():
    try:
        ...
        if not selected_items:
            return
        url = "https://localhost:30003/remove_subscription"
        list_sub = []
...
def subscript(nome, t_min, t_max, hum, pre, window):
    try:
        ...
        if not validate_hp(hum) or not validate_hp(pre):
            ...
            window.lift()
            return
        url = "https://localhost:30003/subscript"
...
def update(password, val_new, data):
    try:
        ...
        if has_no_spaces(val_new):
            ...
            return
        url = "https://localhost:30002/update"
...
def new_form(data):
    ...
    check_temp_min = tk.Checkbutton(form_window, text="Reset", variable=check_var_temp_min,
    ...
    check_temp_max = tk.Checkbutton(form_window, text="Reset", variable=check_var_temp_max,
    ...
    check_var_um = IntVar()
    check_um = tk.Checkbutton(form_window, text="Reset", variable=check_var_um, command=lambda: reset_entry_value(entry_um, check_var_um))
    if isinstance(data, tuple):
        if data[3] == "None":
            check_um.toggle()
            entry_um.insert(0, "")
    ...
    check_precipitazione = tk.Checkbutton(form_window, text="Reset", variable=check_var_precipitazione, command=lambda: reset_entry_value(entry_precipitazione, check_var_precipitazione))
    if isinstance(data, tuple):
        if data[4] == "None":
