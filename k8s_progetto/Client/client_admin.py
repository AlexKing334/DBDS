...
def accedi():
    try:
        ...
        url = "https://localhost:30002/login_admin"
...
def get_violation(type_req, value):
    try:
        url = "https://localhost:30008/get_violation"
...
def get_prediction(dati):
    try:
        ...
        url = "https://localhost:30007/prediction"
...
def get_sla_rules():
    try:
        url = "https://localhost:30008/get_sla_rules"
        payload = {'token': user.getToken()}
        requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
        response = requests.get(url, json=payload, cert=(cert_, key_file), verify=False)
        result = response.json()
...
def update_sla(nome, threshold, is_active, window):
    try:
        url = "https://localhost:30008/update_sla"
...
def update(password, val_new, data):
    try:
        ...
        if has_no_spaces(val_new):
            ...
            return
        url = "https://localhost:30008/update"
